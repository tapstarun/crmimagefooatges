...
class SearchUsers_Functions extends Common_Functions{
	...
	*/
	function getUsers($input_data) { 
		...
		if(isset($input_data->job_title)){
			$where .=" and job_title like '%$input_data->job_title%'";
		}		
		
		if(isset($input_data->business_type)){
			$where .=" and business_type like '%$input_data->business_type%'";
		}		
		...
		$stmt = $this->conn->prepare("SELECT id,email,firstname,job_title,business_type,company,address,phone,mobile,city,state,zipcode,country,'Potential User' as opertionType FROM `potential_users` where $where   union All SELECT id,email,firstname,job_title,business_type,company,address,phone,mobile,city,state,zipcode,country,'advanceUser' as opertionType   FROM `users`  where $where order by id desc ");
//print_r("SELECT id,email,firstname,job_title,business_type,company,address,phone,mobile,city,state,zipcode,country,'Potential User' as User_opration FROM `potential_users`  union All SELECT id,email,firstname,job_title,business_type,company,address,phone,mobile,city,state,zipcode,country,'Client User' as User_opration   FROM `users`  where $where order by id desc ");die;
		if ($stmt->execute()) {				
